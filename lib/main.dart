...
class _ProfileVideoAvatarState extends State<_ProfileVideoAvatar> {
  ...
  Widget build(BuildContext context) {
    ...
    // Offset/scale crop video ini awalnya dikalibrasi terhadap preview crop
    // PERSEGI 280x280 (lihat _VideoCropPageState), lalu dipakai ulang di sini
    // untuk kotak avatar/header yang ukuran & rasio aspeknya berbeda (header
    // profil penuh lebar & tidak persegi). Tanpa margin ekstra ini, translate
    // (pergeseran posisi) bisa mendorong konten yang sudah di-cover keluar
    // dari batas kotak, memunculkan celah/ruang kosong (putih) di salah satu
    // sisi (mis. atas & kanan) karena BoxFit.cover jadi tidak lagi menutupi
    // seluruh area setelah digeser. safeScale memastikan skala selalu cukup
    // besar untuk mengompensasi jarak pergeseran tersebut.
    final double safeScale = max(
      widget.scale,
      1.0 + 2.0 * (widget.offsetX.abs() + widget.offsetY.abs()),
    );
...
class _VideoTrimPageState extends State<_VideoTrimPage> {
  ...
  Future<void> _save() async {
    ...
    final hwCommand = '-y -i "${widget.sourcePath}" -ss $startSec -t $durationSec '
        '-vf "scale=\'min(1280,iw)\':-2" -c:v h264_mediacodec -b:v 8M '
    ...
    final swCommand = '-y -i "${widget.sourcePath}" -ss $startSec -t $durationSec '
        ...
        '-threads 0 -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart "$outputPath"';
...
class Strings {
  static const Map<String, Map<AppLang, String>> _s = {
    ...
    'receipt_empty_hint': {AppLang.en: 'Take a photo or select a receipt to scan automatically', AppLang.id: 'Foto atau pilih struk belanja untuk dipindai otomatis'},
    ...
    'scan_stage_online_unsure': {AppLang.en: 'Online AI result is unsure, trying offline scan...', AppLang.id: 'Hasil AI online kurang yakin, mencoba pemindaian offline...'},
    'scanning_generic': {AppLang.en: 'Scanning...', AppLang.id: 'Memindai...'},
    'read_via_online_ai': {AppLang.en: 'Read via online AI', AppLang.id: 'Dibaca via AI online'},
    'read_offline': {AppLang.en: 'Read offline', AppLang.id: 'Dibaca offline'},
    'source_online_short': {AppLang.en: 'online AI', AppLang.id: 'AI online'},
    'source_offline_short': {AppLang.en: 'offline', AppLang.id: 'offline'},
    'receipt_low_confidence_warning': {AppLang.en: 'Amount & store name were not detected automatically. Please fill them in manually below, or enable online AI for more accurate reading.', AppLang.id: 'Nominal & nama toko tidak terbaca otomatis. Silakan isi manual di bawah, atau aktifkan AI online agar pembacaan lebih akurat.'},
    'open_ai_scan_settings': {AppLang.en: 'Open AI Scan Settings', AppLang.id: 'Buka Pengaturan AI Scan'},
    'store_name_or_title': {AppLang.en: 'Store name / transaction title', AppLang.id: 'Nama toko / judul transaksi'},
    'total_shopping': {AppLang.en: 'Total amount', AppLang.id: 'Total belanja'},
    'detected_shopping_list': {AppLang.en: 'Detected shopping list', AppLang.id: 'Daftar belanja terdeteksi'},
    'view_scanned_text': {AppLang.en: 'View scanned text', AppLang.id: 'Lihat teks hasil pindai'},
    'fill_title_amount_first': {AppLang.en: 'Please fill in the title & amount first', AppLang.id: 'Lengkapi judul & nominal terlebih dahulu'},
    'receipt_transaction_saved': {AppLang.en: 'Transaction from receipt saved', AppLang.id: 'Transaksi dari struk tersimpan'},
    'receipt_note_header': {AppLang.en: 'Receipt scan result ({source})', AppLang.id: 'Hasil pindai struk ({source})'},
    'receipt_note_shopping_list_label': {AppLang.en: 'Shopping list:', AppLang.id: 'Daftar belanja:'},
    'ai_settings': {AppLang.en: 'AI Scan Settings', AppLang.id: 'Pengaturan AI Scan'},
    'use_ai_online': {AppLang.en: 'Use Online AI', AppLang.id: 'Gunakan AI Online'},
    'ai_online_desc': {AppLang.en: 'Online AI is more accurate. If it fails, it automatically falls back to offline.', AppLang.id: 'AI online lebih akurat. Jika gagal, otomatis beralih ke offline.'},
    'ai_provider_label': {AppLang.en: 'AI PROVIDER', AppLang.id: 'PROVIDER AI'},
    'api_key_model_label': {AppLang.en: '{provider} API KEY & MODEL', AppLang.id: 'API KEY & MODEL {provider}'},
    'api_keys_label': {AppLang.en: 'API Keys', AppLang.id: 'API Keys'},
    'keys_configured_count': {AppLang.en: '{count} configured', AppLang.id: '{count} terpasang'},
    'manage_api_key': {AppLang.en: 'Manage API Key', AppLang.id: 'Kelola API Key'},
    'model_label': {AppLang.en: 'Model', AppLang.id: 'Model'},
    'no_models_yet': {AppLang.en: 'No models yet', AppLang.id: 'Belum ada model'},
    'update_model_list_desc': {AppLang.en: 'Refresh the model list based on your {provider} API key.', AppLang.id: 'Perbarui daftar model sesuai API key {provider} Anda.'},
    'api_key_privacy_note': {AppLang.en: 'The API key and model are stored locally and only sent to the AI provider while scanning.', AppLang.id: 'API key dan model disimpan lokal dan hanya dikirim ke penyedia AI saat memindai.'},
    'test_connection': {AppLang.en: 'Test Connection', AppLang.id: 'Tes Koneksi'},
    'save': {AppLang.en: 'Save', AppLang.id: 'Simpan'},
    'save_api_key': {AppLang.en: 'Save API Key', AppLang.id: 'Simpan API Key'},
    'api_key_for_provider': {AppLang.en: '{provider} API Key', AppLang.id: 'API Key {provider}'},
    'api_key_manager_desc': {AppLang.en: 'Add several API keys as backup in case a quota limit is reached.', AppLang.id: 'Tambahkan beberapa API key sebagai cadangan jika limit kuota tercapai.'},
    'api_key_numbered': {AppLang.en: 'API key {n}', AppLang.id: 'API key {n}'},
    'delete_api_key_numbered': {AppLang.en: 'Delete API key {n}', AppLang.id: 'Hapus API key {n}'},
    'add_api_key': {AppLang.en: 'Add API key', AppLang.id: 'Tambah API key'},
    'settings_saved_no_key': {AppLang.en: 'Settings saved (no API key filled in yet)', AppLang.id: 'Pengaturan disimpan (belum ada API key terisi)'},
    'api_key_model_saved': {AppLang.en: 'API key & model saved', AppLang.id: 'API key & model disimpan'},
    'enter_api_key_first': {AppLang.en: 'Enter at least one API key first', AppLang.id: 'Masukkan minimal satu API key terlebih dahulu'},
    'load_select_model_first': {AppLang.en: 'Load & select a model first', AppLang.id: 'Muat & pilih model terlebih dahulu'},
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        inputDecorationTheme: InputDecorationTheme(
          // Fill lama (F1EEF7, lalu tint tipis 0.07) masih nyaris menyatu
          // dengan scaffoldBackgroundColor di mode terang. Opacity tint
          // dinaikkan cukup jauh (0.16) supaya kotak input benar-benar
          // tampak lebih gelap & kontras terhadap latar, bukan hanya
          // mengandalkan border tipis untuk membedakannya.
          filled: true, fillColor: Color.alphaBlend(theme.primary.withOpacity(0.16), const Color(0xFFFFFFFF)),
      ...
      darkTheme: ThemeData(
        ...
        scaffoldBackgroundColor: Color.alphaBlend(theme.primary.withOpacity(0.03), const Color(0xFF1E1B27)),
        colorScheme: ColorScheme.fromSeed(
          seedColor: theme.primary,
          brightness: Brightness.dark,
        ).copyWith(
...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  final slideKeys = const [
    ('onboarding_title_1', 'onboarding_desc_1', SolarIconsBold.wallet),
    ('onboarding_title_2', 'onboarding_desc_2', SolarIconsBold.billList),
    ('onboarding_title_3', 'onboarding_desc_3', SolarIconsBold.chart),
  ];
              ...
              child: Row(
                ...
                children: [
                  ...
                  AnimatedOpacity(
                    ...
                    child: TextButton(
                      onPressed: _currentPage < 3 ? () => _pageController.animateToPage(3, duration: const Duration(milliseconds: 600), curve: Curves.easeOutCubic) : null,
                      child: Text(Strings.t(lang, 'skip')),
                    ),
                  ),
                ],
              ),
            ...
            Expanded(
              child: PageView.builder(
                ...
                itemBuilder: (context, i) {
                  if (i < 3) {
                    return _buildSlide(i, colors, lang, isDark);
                  } else {
                    return _buildUsernameSlide(colors, lang, isDark);
                  }
                },
              ),
            ),
              ...
              child: Row(
                children: [
                  ...List.generate(4, (i) => AnimatedContainer(
                        ...
                        width: i == _currentPage ? 28 : 7,
                        height: 7,
                        decoration: BoxDecoration(
                          color: i == _currentPage ? colors.primary : (isDark ? Colors.white24 : colors.tertiary),
                  ...
                  FilledButton.icon(
                    ...
                    icon: Icon(_currentPage == 3 ? Icons.check : SolarIconsOutline.arrowRight),
                    label: Text(_currentPage == 3 ? Strings.t(lang, 'finish') : (_currentPage == 2 ? Strings.t(lang, 'get_started') : Strings.t(lang, 'continue'))),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildSlide(int index, ColorScheme colors, AppLang lang, bool isDark) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Container(
              ...
              child: Stack(
                children: [
                  ...
                  Center(child: Icon(slideKeys[index].$3, size: 94, color: Colors.white)),
                ],
              ),
            ),
          ),
          ...
          StaggeredReveal(
            ...
            animate: true,
            stagger: true,
            child: Text(Strings.t(lang, slideKeys[index].$1), style: TextStyle(fontFamily: 'DM Serif Display', fontSize: 36, height: 1.08, color: context.textPrimary)),
          ),
          ...
          StaggeredReveal(
            ...
            animate: true,
            stagger: true,
            child: Text(Strings.t(lang, slideKeys[index].$2), style: TextStyle(fontSize: 16, height: 1.5, color: context.textMuted)),
          ),
        ],
      ),
    );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 40),
          StaggeredReveal(
            key: const ValueKey('slide_title_3'),
            index: 1,
            animate: true,
            stagger: true,
            child: Text(Strings.t(lang, 'set_username_title'), style: TextStyle(fontFamily: 'DM Serif Display', fontSize: 36, height: 1.08, color: context.textPrimary)),
          ),
          const SizedBox(height: 16),
          StaggeredReveal(
            key: const ValueKey('slide_desc_3'),
            index: 2,
            animate: true,
            stagger: true,
            child: Text(Strings.t(lang, 'set_username_desc'), style: TextStyle(fontSize: 16, height: 1.5, color: context.textMuted)),
          ),
          const SizedBox(height: 24),
          StaggeredReveal(
            key: const ValueKey('slide_input_3'),
            index: 3,
            animate: true,
            stagger: true,
            child: ShakeField(
...
class _HeaderSnapScrollPhysics extends ScrollPhysics {
  ...
  double applyBoundaryConditions(ScrollMetrics position, double value) {
    ...
    // berbeda dari batas atas Fase 2 yang sengaja tetap elastis.
  ...
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
    ...
    // semakin besar hambatannya (mendekati maxTopRubberOffset). Batas
    // bawah tidak lagi punya efek rubber sama sekali (lihat
    // applyBoundaryConditions yang mengunci keras di maxScrollExtent).
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Transform.scale(
            ...
            child: NotificationListener<ScrollNotification>(
              ...
              physics: _HeaderSnapScrollPhysics(
                ...
                parent: const ClampingScrollPhysics(parent: AlwaysScrollableScrollPhysics()),
              ),
              slivers: [
                SliverToBoxAdapter(
          ...
          // di Layer 1 (di BAWAH Layer 2/CustomScrollView). Scrollable memakai
          // HitTestBehavior.opaque untuk seluruh area scroll-nya (agar bisa
          // di-drag dari mana saja untuk expand/collapse header), sehingga
          // tombol-tombol itu tidak pernah menerima tap sama sekali. Layer ini
          // menduplikasi HANYA target tap kecil tersebut di layer paling atas,
          // supaya bisa di-tap, sementara gestur drag expand/collapse di area
          // lain header tetap berfungsi normal.
          Positioned(
            top: 0, left: 0, right: 0, bottom: 0,
            child: Transform.translate(
              offset: Offset(0, -parallaxOffset),
...
class _ProfileHeaderMediaContent extends StatelessWidget {
  ...
  Widget _fallback(BuildContext context, double refSize) {
    ...
    return Container(
      color: primary,
      alignment: Alignment.center,
      child: Text(initial, style: TextStyle(color: Colors.white, fontSize: refSize * 0.26, fontWeight: FontWeight.bold)),
    );
  }

  Widget _photoContent(BuildContext context, double refSize) {
    if (kIsWeb) {
      if (profile.photoBytesBase64 == null) return _fallback(context, refSize);
      try {
        final bytes = base64Decode(profile.photoBytesBase64!);
        return Image.memory(bytes, fit: BoxFit.cover, gaplessPlayback: true, errorBuilder: (c, e, s) => _fallback(context, refSize));
      } catch (_) {
        return _fallback(context, refSize);
      }
    }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      final refSize = constraints.maxWidth.isFinite ? constraints.maxWidth : 280.0;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AnimatedSize(
      ...
      child: AnimatedOpacity(
        ...
        curve: Curves.easeOut,
        // PENTING: child TIDAK boleh diganti dengan widget lain (mis. SizedBox kosong)
        // saat isRemoving, karena itu membuat BuildContext dari tile ini ter-unmount
        // sebelum callback penghapusan (Future.delayed) sempat berjalan. Akibatnya
        // context.mounted menjadi false, ref.read(transactionsProvider.notifier).remove()
        // batal dipanggil, transaksi tidak benar-benar terhapus, dan saldo utama tidak
        // ter-update meski item sudah tidak terlihat di daftar.
        child: IgnorePointer(
          ignoring: isRemoving,
          child: ConstrainedBox(
            constraints: isRemoving ? const BoxConstraints(maxHeight: 0) : const BoxConstraints(),
...
const SizedBox(width: 16),
ScrollFadeTitle(controller: _scrollController, child: Text(Strings.t(lang, 'language'), style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: context.textPrimary))),
                  ],
                ),
              ),
            ),
    ...
    pageBuilder: (dialogContext, animation, secondaryAnimation) => Dialog(
      ...
      child: GlassContainer(
        ...
        settings: LiquidGlassSettings(
          blur: 12,
          thickness: 16,
          glassColor: isDark ? const Color.fromRGBO(0, 0, 0, 0.5) : const Color.fromRGBO(255, 255, 255, 0.35),
          whitenStrength: 0.0,
          lightIntensity: isDark ? 0.12 : 0.6,
          ambientStrength: isDark ? 0.08 : 0.35,
          saturation: 1.3,
          refractiveIndex: isDark ? 0.15 : 1.2,
          chromaticAberration: 0.0,
          fresnelStrength: 0.0,
          glowIntensity: 0.0,
          shadowElevation: isDark ? 1 : 2,
        ),
        child: Padding(
...
const SizedBox(width: 16),
Expanded(
  child: ScrollFadeTitle(controller: _scrollController, child: Text(Strings.t(lang, 'loans_title'), style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: context.textPrimary))),
),
                    GestureDetector(
